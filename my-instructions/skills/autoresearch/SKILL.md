...
6. **Budget cap** — Optional. Max number of experiment cycles before stopping. Default: 20. (Set to 0 to go uncapped — use with caution.)
...
**Vision evals — for skills that produce images or visual output:**
If the skill outputs image files, add at least one vision-based eval. After each run, use the `view_image` tool to inspect the generated file and answer a binary question about it. Keep vision evals tightly scoped — the question must be answerable with yes/no from a single glance:
- ✅ Good: "Does the image visually match the initial prompt (correct subject, scene, and mood as described)?" / "Does the image match the requested aspect ratio (portrait vs landscape)?" / "Is there visible text in the image?"
- ❌ Bad: "Is this image high quality?" / "Does this look professional?" / "Does the image contain the subject" (too loose — check against the *specific* prompt, not just presence of any subject)

Format vision evals identically to other evals, but add `[vision]` to the name:
```
EVAL [number]: [Short name] [vision]
Question: [Yes/no question answerable by looking at the image]
Pass condition: [What you see that means "yes"]
Fail condition: [What you see that means "no" or image missing/failed to generate]
```
...
> ⚠️ **DATA LOADING — READ THIS FIRST:** Do NOT use `fetch()`, `XMLHttpRequest`, or any network call to load data. Browsers block all network requests on `file://` URLs — the dashboard will silently show nothing. Do NOT use `<script src="...">` CDN links either — if the CDN fails or the machine is offline, the entire JS runtime crashes before rendering anything. The only approach that works: embed all data in the HTML inside a `<script id="results-data" type="application/json">` tag and read it with `JSON.parse(document.getElementById('results-data').textContent)`. Update that tag in-place after each experiment. No CDN. No fetch. No polling.
...
Generate the dashboard as a single self-contained HTML file with inline CSS and JavaScript. Draw the score progression chart as an inline SVG using JavaScript — no CDN chart libraries. Call `render()` inside a `DOMContentLoaded` listener so the DOM is ready before parsing the embedded JSON.

**Open it immediately** after creating it: `open dashboard.html` (macOS) so the user can see it in their browser.

**Update the embedded `results-data` script tag** after every experiment so the dashboard stays current when the user reloads. The JSON format:
...
1. **Auto-detect the version number.** Look inside the skill's folder for existing version directories named `[skill-name]-v2`, `[skill-name]-v3`, etc. Take the highest N found and increment by 1. If none exist, start at `v2`. Never ask the user to name the version — always derive it automatically. Example: skill is `paid-ads`, folders `paid-ads-v2/` and `paid-ads-v3/` exist → new version is `paid-ads-v4`.
2. Create two directories inside the skill's folder:
   - `autoresearch-[skill-name]/` — working directory for results, dashboard, and changelog
   - `[skill-name]-vN/` — output directory for the optimized skill (e.g. `paid-ads-v4/`)
3. **Copy the original SKILL.md into `[skill-name]-vN/SKILL.md`** — this is the copy you will mutate. NEVER edit the original SKILL.md. All mutations happen on this copy only.
4. Also save `SKILL.md.baseline` in `autoresearch-[skill-name]/` (identical to the original — this is your revert target)
5. Create `results.tsv` with the header row
6. Create `dashboard.html`, then open the dashboard in the browser
7. Run the skill [N] times using the test inputs (use `[skill-name]-vN/SKILL.md` for all runs)
...
3. **Make the change.** Edit `[skill-name]-vN/SKILL.md` with ONE targeted mutation. NEVER touch the original SKILL.md.
...
6. **Decide: keep or discard.**
   - Score improved → **KEEP.** Log it. This is the new baseline for `[skill-name]-vN/SKILL.md`.
   - Score stayed the same → **DISCARD.** Revert `[skill-name]-vN/SKILL.md` to previous version. The change added complexity without improvement.
   - Score got worse → **DISCARD.** Revert `[skill-name]-vN/SKILL.md` to previous version.
...
6. **The improved `[skill-name]-vN/SKILL.md`** (the original SKILL.md is untouched)
7. **Location of results.tsv and changelog.md** in `autoresearch-[skill-name]/` for reference

---

## output format

The skill produces files across two sibling directories inside the target skill's folder:

```
[skill-name]/
├── SKILL.md                        # original — NEVER touched
├── [skill-name]-v2/
│   └── SKILL.md                    # first optimized version
├── [skill-name]-v3/
│   └── SKILL.md                    # second optimized version (if run again)
└── autoresearch-[skill-name]/
    ├── dashboard.html              # live browser dashboard
    ├── results.tsv                 # score log for every experiment
    ├── changelog.md                # detailed mutation log
    └── SKILL.md.baseline           # snapshot of original before optimization
```

**The original SKILL.md is NEVER modified.** The improved version lives in `[skill-name]-vN/SKILL.md`. Version folders accumulate each time the skill is run — always auto-increment, never overwrite a previous version. The user can review, diff, and manually promote any version if they choose.
